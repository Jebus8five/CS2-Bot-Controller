...
// Creates the synchronous console and file sinks.
bool Init(const char* baseDir, char* error, size_t maxlen)
{
    try
    {
        const auto directory = std::filesystem::path(baseDir) / "addons/BotController/logs";
        std::filesystem::create_directories(directory);
        std::vector<spdlog::sink_ptr> sinks;
        sinks.emplace_back(std::make_shared<spdlog::sinks::stderr_color_sink_mt>());
        sinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>((directory / "BotController.log").string(), false));
        sinks[0]->set_pattern("%^[%T.%e] %n: %v%$");
        sinks[1]->set_pattern("[%Y-%m-%d %T.%e] [%l] %n: %v");
        g_logger = std::make_shared<spdlog::logger>("BotController", sinks.begin(), sinks.end());
    ...
    {
        std::snprintf(error, maxlen, "cannot initialize BotController logger: %s", exception.what());
        Close();
        return false;
    }
}
