...
public partial class BotControllerImplSW2Plugin
{
    ...
    private readonly List<Guid> _projectileEventHooks = new();

    // Registers entity, tick, and detonation handlers used by projectile replay
    private void RegisterProjectileEvents()
    {
        Core.Event.OnEntitySpawned += OnProjectileEntitySpawned;
        Core.Event.OnTick += ProcessPendingProjectileCandidates;
        _projectileEventHooks.Add(Core.GameEvent.HookPost<EventSmokegrenadeDetonate>(OnSmokegrenadeDetonate));
        _projectileEventHooks.Add(Core.GameEvent.HookPost<EventFlashbangDetonate>(OnFlashbangDetonate));
        _projectileEventHooks.Add(Core.GameEvent.HookPost<EventHegrenadeDetonate>(OnHegrenadeDetonate));
        _projectileEventHooks.Add(Core.GameEvent.HookPost<EventMolotovDetonate>(OnMolotovDetonate));
        _projectileEventHooks.Add(Core.GameEvent.HookPost<EventDecoyDetonate>(OnDecoyDetonate));
    }

    // Unregisters projectile handlers and clears their managed state
    private void UnregisterProjectileEvents()
    {
        Core.Event.OnEntitySpawned -= OnProjectileEntitySpawned;
        Core.Event.OnTick -= ProcessPendingProjectileCandidates;
        foreach (Guid hook in _projectileEventHooks) Core.GameEvent.Unhook(hook);
        _projectileEventHooks.Clear();
    ...
    private void OnProjectileEntitySpawned(IOnEntitySpawnedEvent @event)
    {
        ...
        var candidate = new PendingProjectileCandidate
        {
            ...
            EntityPtr = ((INativeHandle)entity).Address,
            Kind = kind,
    ...
    private bool TryProcessProjectileCandidate(PendingProjectileCandidate candidate)
    {
        CBaseCSGrenadeProjectile? projectile =
            Core.EntitySystem.GetEntityByIndex<CBaseCSGrenadeProjectile>(candidate.EntityIndex);
        if (projectile is not { IsValid: true }) return true;
    ...
    private bool TryCandidateSlot(PendingProjectileCandidate candidate, out int slot)
    {
        slot = -1;
        CBaseCSGrenadeProjectile? projectile =
            Core.EntitySystem.GetEntityByIndex<CBaseCSGrenadeProjectile>(candidate.EntityIndex);
        return projectile is { IsValid: true } && TryGetProjectileThrowerSlot(projectile, out slot);
    }
        ...
        PendingProjectileCandidate candidate)
    {
        ...
        if (!BotController.QueueProjectileBirthAlign(
                ((INativeHandle)projectile).Address,
    ...
    private bool TryGetProjectileThrowerSlot(CBaseCSGrenadeProjectile projectile, out int slot)
    {
        slot = -1;
        CCSPlayerPawn? thrower = projectile.Thrower.Value;
        if (thrower is not { IsValid: true }) return false;

        IPlayer? player = Core.PlayerManager.GetPlayerFromPawn(thrower);
        if (player is not { IsValid: true }) return false;
        slot = player.Slot;
        return true;
    }

    // Converts a Swiftly vector into stable recording storage
    ...
    {
        CaptureProjectileDetonation(@event.UserIdPlayer, ReplayProjectileKind.Smoke, @event.X, @event.Y, @event.Z);
    ...
    {
        CaptureProjectileDetonation(@event.UserIdPlayer, ReplayProjectileKind.Flash, @event.X, @event.Y, @event.Z);
        return HookResult.Continue;
    }
    ...
    {
        CaptureProjectileDetonation(@event.UserIdPlayer, ReplayProjectileKind.He, @event.X, @event.Y, @event.Z);
        return HookResult.Continue;
    }
    ...
    {
        CaptureProjectileDetonation(@event.UserIdPlayer, ReplayProjectileKind.Molotov, @event.X, @event.Y, @event.Z);
        return HookResult.Continue;
    }

    // Captures decoy detonation position for the latest recorded decoy
    private HookResult OnDecoyDetonate(EventDecoyDetonate @event)
    {
        CaptureProjectileDetonation(@event.UserIdPlayer, ReplayProjectileKind.Decoy, @event.X, @event.Y, @event.Z);
        return HookResult.Continue;
    }
}
